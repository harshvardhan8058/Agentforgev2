...
- [x] 2. Tool interface and registry
  - [x] 2.1 Implement the Tool interface and value types
  ...
  - [x] 2.2 Implement the Tool_Registry
  ...
  - [x]* 2.3 Write property test for registry register/resolve/list round-trip
    - **Property 5: Tool registry register/resolve/list round-trip**
    - **Validates: Requirements 2.2, 2.3**

  - [x]* 2.4 Write property test for duplicate tool-name rejection
    - **Property 6: Duplicate tool-name registration is rejected**
    - **Validates: Requirements 2.4**

  - [x]* 2.5 Write smoke test asserting the Tool interface is abstract
    - Assert `Tool_Interface` cannot be instantiated and declares the required abstract
      members
    - _Requirements: 2.1_

- [x] 3. Agent run state and Iteration_Limit resolution
  - [x] 3.1 Implement AgentState and termination types
  ...
  - [x] 3.2 Implement Iteration_Limit resolution
  ...
  - [x]* 3.3 Write property test for Iteration_Limit resolution
    - **Property 4: Iteration_Limit resolution**
    - **Validates: Requirements 1.5, 1.6**

- [x] 4. Tool-call parsing and deterministic fallback selection
  - [x] 4.1 Implement decision parsing and the Selection_Strategy seam
  ...
  - [x]* 4.2 Write unit test for decision parsing and fallback selection
...
- [x] 5. Built-in RAG retrieval tool
  - [x] 5.1 Implement the RAG_Tool
  ...
  - [x]* 5.2 Write property test for RAG_Tool answer/citation mirroring
    - **Property 12: RAG_Tool mirrors the RAG_Service answer and citations**
    - **Validates: Requirements 4.3**

  - [x]* 5.3 Write unit test that RAG_Tool delegates to RAG_Service
    - With a stub `RAG_Service`, assert `invoke` calls it once and does not perform its own
      retrieval/generation
    - _Requirements: 4.2, 12.2_

- [x] 6. Built-in web search tool with graceful degradation
  - [x] 6.1 Implement the Search_Provider interface and disabled default
  ...
  - [x] 6.2 Implement the Web_Search_Tool
  ...
  - [x]* 6.3 Write property test for disabled web search
    - **Property 11: Disabled web search returns an unavailable result without network access**
    - **Validates: Requirements 5.4, 5.5**

- [x] 7. Agent orchestrator LangGraph graph
  - [x] 7.1 Implement the reason/act/observe nodes and edge routing
  ...
  - [x] 7.2 Implement the Agent_Orchestrator
...
- [x] 9. Short-term memory
  - [x] 9.1 Implement the Memory_Manager interface and short-term memory
  ...
  - [x]* 9.2 Write property test for the short-term size invariant + FIFO + retention
    - **Property 13: Short-term memory size invariant with request retention and FIFO eviction**
    - **Validates: Requirements 6.4, 6.5, 6.6**

  - [x]* 9.3 Write property test for invalid Size_Budget rejection
    - **Property 14: Invalid Size_Budget is rejected**
    - **Validates: Requirements 6.3**

- [x] 10. Long-term memory
  - [x] 10.1 Implement long-term memory over existing seams
  ...
  - [x]* 10.2 Write property test for long-term top-K bound and ordering
    - **Property 15: Long-term memory top-K bound and ordering**
    - **Validates: Requirements 7.2, 7.3, 7.4**

- [x] 11. Persistent conversation history
  - [x] 11.1 Add the conversations migration
  ...
  - [x] 11.2 Implement the Conversation_Store
  ...
  - [x]* 11.3 Write property test for conversation append/history ordering with auto-create
    - **Property 16: Conversation append/history ordering with auto-create**
    - **Validates: Requirements 8.2, 8.3, 8.4**

  - [x]* 11.4 Write unit tests for conversation-id uniqueness and final-message persistence
    - Assert `create` yields unique ids and that a completed run appends the final
      assistant message
    - _Requirements: 8.1, 8.5_

- [x] 12. Observability tracing of agent steps
  - [x] 12.1 Add the agent-traces migration
  ...
  - [x] 12.2 Implement the Trace_Recorder
  ...
  - [x]* 12.3 Write property test for trace completeness and ordering
    - **Property 18: Trace is complete and ordered by ordinal**
    - **Validates: Requirements 10.1, 10.2, 10.3**

- [x] 13. Streaming responses (SSE)
  - [x] 13.1 Implement the Streaming_Service
  ...
  - [x]* 13.2 Write property test for streaming terminal-event and ordering guarantees
    - **Property 17: Streaming emits exactly one terminal event and preserves order**
    - **Validates: Requirements 9.3, 9.4, 9.5, 9.6, 9.8, 9.9**

  - [x]* 13.3 Write integration test for SSE first-event timing and incremental emission
...
- [x] 14. Fallback determinism (end-to-end)
  - [x]* 14.1 Write property test for fallback determinism of answer and event sequence
    - **Property 19: Fallback determinism of answer and event sequence**
    - **Validates: Requirements 1.8, 3.5, 9.7**

- [x] 15. Composition root extension and wiring
  - [x] 15.1 Implement build_agent_context and provider/tool wiring
  ...
  - [x]* 15.2 Write unit tests for search registration policy and seam-only dependencies
...
- [x] 16. API endpoints and router registration
  - [x] 16.1 Implement the conversations router and schemas
  ...
  - [x] 16.2 Implement the agent router and schemas
  ...
  - [x] 16.3 Register Phase 3 routers in the app factory
  ...
  - [x]* 16.4 Write keyless integration tests for /agent/run and /agent/stream
  ...
  - [x]* 16.5 Write unit test for the error-envelope shape on agent error paths
    - Assert unknown conversation/run ids return `404 not_found` through the existing
      envelope
    - _Requirements: 12.4_

- [x] 17. Documentation of agentic-layer design decisions
  - [x] 17.1 Write the agentic-layer decision record
...
- [x] 18. Checkpoint — documentation and integration verified
...
- [x] 19. Final checkpoint — full keyless suite green
