...
- [ ] 1. Extend `Settings`, scaffold the `enterprise/` package with base interfaces and stubs
  ...
  - [ ]* 1.1 Write a smoke test for the package layout, ABC abstractness, and settings defaults
...
- [ ] 2. Implement enterprise domain models and `RBAC_Policy`, with property tests
  - [ ] 2.1 Implement `enterprise/models.py` domain dataclasses
  ...
  - [ ] 2.2 Implement `enterprise/rbac.py` — Role/Permission enums, `ROLE_PERMISSIONS`, `RBAC_Policy`
  ...
  - [ ]* 2.3 Write property test for the RBAC iff-invariant
  ...
  - [ ]* 2.4 Write property test for role-permission subset nesting with universal read
...
- [ ] 3. Implement `Auth_Service` — password hashing + JWT round-trip
  - [ ] 3.1 Add dependencies and implement `enterprise/auth.py`
  ...
  - [ ]* 3.2 Write property test for JWT issue/verify round-trip and rejection of tampered tokens
  ...
  - [ ]* 3.3 Write property test for password hash correctness and plaintext non-disclosure
  ...
  - [ ]* 3.4 Write unit tests for hash + verify basics
    - Cover the argon2 `VerifyMismatchError` path, unicode passwords, empty strings, and
      long inputs.
    - _Requirements: 1.1, 1.6_

- [ ] 4. Implement `Identity_Store` (in-memory + Postgres) and migration `0006`
  - [ ] 4.1 Finalize the `Identity_Store` ABC in `enterprise/base.py`
  ...
  - [ ] 4.2 Implement `InMemory_Identity_Store` in `enterprise/identity.py`
  ...
  - [ ] 4.3 Implement `Pg_Identity_Store` in `enterprise/identity.py`
  ...
  - [ ] 4.4 Add migration `migrations/0006_create_enterprise_identity.sql`
  ...
  - [ ]* 4.5 Write unit tests for `InMemory_Identity_Store` invariants
  ...
  - [ ]* 4.6 Write an integration test applying migration `0006` and round-tripping identity (`@pytest.mark.integration`)
...
- [ ] 5. Implement `API_Key_Service` (`enterprise/api_keys.py`) with in-memory + Postgres stores
  - [ ] 5.1 Finalize the `API_Key_Store` ABC in `enterprise/base.py`
  ...
  - [ ] 5.2 Implement `API_Key_Service.create`
  ...
  - [ ] 5.3 Implement `resolve_key`, `list`, and `revoke`
  ...
  - [ ] 5.4 Provide `InMemory_API_Key_Store` and `Pg_API_Key_Store`
  ...
  - [ ]* 5.5 Write property test for the API-key lifecycle
  ...
  - [ ]* 5.6 Write unit tests for the one-time-return and list shape
...
- [ ] 6. Implement `Rate_Limiter` (`enterprise/rate_limit.py`)
  - [ ] 6.1 Implement the `Rate_Limiter` ABC and three concrete implementations
  ...
  - [ ]* 6.2 Write property test for the rate limiter's bound + per-principal isolation + rollover
  ...
  - [ ]* 6.3 Write a unit test for window rollover under `Fake_Clock_Rate_Limiter`
...
- [ ] 7. Implement `Principal` + FastAPI dependencies (`enterprise/principal.py` + `api/deps.py`)
  - [ ] 7.1 Wire `Principal` and `get_current_principal`
  ...
  - [ ] 7.2 Implement `require_permission(permission)` dependency factory + `get_org_id`
  ...
  - [ ]* 7.3 Write property test for principal resolution
  ...
  - [ ]* 7.4 Write unit tests for the dependency chain
...
- [ ] 8. Wire the enterprise components through the composition root (`config/container.py`)
  ...
  - [ ]* 8.1 Write unit tests for the composition root selection
...
- [ ] 9. Add tenancy migration `0007_add_org_id_to_tenant_resources.sql`
  ...
  - [ ]* 9.1 Write an integration test for migration `0007` (`@pytest.mark.integration`)
...
- [ ] 10. Extend every existing tenant-owned store to require and constrain by `org_id`
  - [ ] 10.1 Extend the store interfaces + Postgres implementations
  ...
  - [ ] 10.2 Extend the in-memory counterparts
  ...
  - [ ] 10.3 Extend the vector-store / long-term memory writes and queries with `org_id`
  ...
  - [ ] 10.4 Thread `org_id` through the services that persist tenant-owned rows
  ...
  - [ ]* 10.5 Write property test for tenant isolation across every resource type
  ...
  - [ ]* 10.6 Write property test for the tenant-scoped store round-trip
  ...
  - [ ]* 10.7 Write unit tests for cross-tenant `None` on each in-memory store
  ...
  - [ ]* 10.8 Write an integration test round-tripping org-scoped documents/conversations/agent_runs/multi_agent_runs (`@pytest.mark.integration`)
...
- [ ] 11. Implement the new `auth` router (`api/routers/auth.py`)
  ...
  - [ ]* 11.1 Write unit tests for the auth router error branches
...
- [ ] 12. Implement the new `orgs` router (`api/routers/orgs.py`)
  ...
  - [ ]* 12.1 Write unit tests for the orgs router
...
- [ ] 13. Apply auth + tenancy to the existing routers (`ingest`, `query`, `documents`, `conversations`, `agent`, `multi_agent`)
  ...
  - [ ]* 13.1 Write property test for the endpoint permission mapping
  ...
  - [ ]* 13.2 Write unit tests for 401 / 403 / cross-tenant 404 on the extended routers
...
- [ ] 14. Register the new routers and wire the enterprise context in `main.py`
...
- [ ] 15. Extend `docs/decisions.md` with the Phase 5 design decisions
...
- [ ] 17. Final full-suite keyless checkpoint (leave unchecked for the user)
