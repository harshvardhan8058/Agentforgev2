...
- **Limitations:** no durable queue (in-process, best-effort), a streamed run's connection stays
  open until its post-stream hook returns, DNS rebinding between admission and connect is not
  closed, the delivery log has no retention bound, and guardrail blocks are emitted for the
  non-streaming surfaces only. See `docs/KNOWN_LIMITATIONS.md` and `docs/WEBHOOKS.md`.
