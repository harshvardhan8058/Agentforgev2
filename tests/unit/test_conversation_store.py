...
def test_create_yields_unique_conversation_ids():
    ...
    ids = {store.create(ORG) for _ in range(50)}
    assert len(ids) == 50
...
def test_final_assistant_message_persistence_pattern():
    ...
    store = InMemory_Conversation_Store()
    conversation_id = store.create(ORG)

    store.append(ORG, conversation_id, "user", "what is agentforge?")
    final = store.append(ORG, conversation_id, "assistant", "AgentForge is a platform.")
...
def test_append_auto_creates_unknown_conversation():
    ...
    store = InMemory_Conversation_Store()
    message = store.append(ORG, "brand-new-id", "user", "hello")
    assert message.position == 0
    assert [m.content for m in store.history(ORG, "brand-new-id")] == ["hello"]
