...
function CommandTrigger(): JSX.Element {
  ...
  return (
    <button
      ...
      className="inline-flex items-center gap-2 rounded-md border border-border bg-surface px-3 py-1.5 text-sm text-text-muted hover:border-border-strong focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus-ring"
    >
      <Search className="h-4 w-4" aria-hidden="true" />
      <span className="hidden sm:inline">Search…</span>
      <kbd className="hidden rounded border border-border bg-surface-raised px-1.5 text-xs text-text-muted sm:inline">
        ⌘K
      </kbd>
    </button>
  );
    ...
    <div className="flex min-h-screen bg-bg text-text" data-testid="app-shell">
      ...
      {isDesktop && (
        <aside
          data-testid="sidebar"
          className={cn(
            "sticky top-0 flex h-screen shrink-0 flex-col gap-4 border-r border-border bg-bg-subtle p-3 transition-[width]",
            collapsed ? "w-16" : "w-60",
          )}
        >
          ...
          <div className="mt-auto px-1">
            <button
              ...
              className="inline-flex h-8 w-8 items-center justify-center rounded-md text-text-muted hover:bg-surface-raised hover:text-text focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus-ring"
            >
